...
function installTavernInput() {
  ...
  button.id = 'send_but';
  button.type = 'button';
  const send = () => { if (typeof window.sendUserMessage === 'function') window.sendUserMessage(textarea.value); };
...
window.addEventListener('message', async event => {
  ...
  try {
    ...
    for (let index = 0; index < parent.length; index++) {
      try { if (parent[index].__tavernHubId === event.data.hubId) { hub = parent[index]; break; } } catch {}
    }
    if (!hub) throw new Error('当前角色前端尚未准备好');
    hub.__tavernExpose(window, event.data.index);
    installTavernInput();
    const frame = document.createElement('iframe');
    frame.title = '角色卡前端组件'; frame.scrolling = 'no';
