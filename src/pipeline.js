...
import { legacyPromptMessages, toolPresetMessages } from './presets.js';
import { hasSummarySelection, runAdvancePreset } from './advance.js';
...
export async function mapConcurrent(values, limit, run) {
  ...
  await Promise.all(Array.from({ length: Math.min(limit, values.length) }, async () => {
    while (next < values.length) { const index = next++; result[index] = await run(values[index], index); }
  }));
  return result;
}
...
export async function buildRoster({ state, card, extraBooks, callModel, text = '', emit = () => {}, signal, toolPreset = null, agentPresets = {} }) {
  ...
  const result = await callModel({ stage: 'roster', label: '识别角色', agent: state.config.agents.combine, schema: ROSTER, retries: state.config.protocolRetries, signal, validate: checkRoster, messages: [
    ...
    { role: 'user', content: JSON.stringify({ card: { name: card.name, description: card.description, personality: card.personality, scenario: card.scenario, firstMessage: card.first_mes }, userInput: text, worldbook: relevant.map(e => ({ id: e.id, title: e.comment, keys: e.keys, content: e.content })) }) },
  ] });
  checkRoster(result);
  const names = new Set();
  const characters = result.characters.map(character => {
    ...
    return { ...previous, ...character, id: previous?.id ?? randomUUID(), enabled: previous?.enabled ?? true };
  });
...
export async function runTurn({ state: inputState, card, preset, toolPreset = null, agentPresets = {}, legacy, extraBooks = [], advanceWorldbooks, regex = [], callModel, text, trigger = 'normal', signal, emit = () => {}, scanWorldbook, beforeWrite, afterWrite, beforeCommit }) {
  ...
  const userMessage = { id: randomUUID(), role: 'user', name: state.userName, content: text, createdAt: state.turnStartedAt, turnId };
  ...
  const stageCall = async (stage, input, schema, label = stage, extras = [], override, validate) => {
    ...
    const usage = [];
    const messages = [{ role: 'system', content: [ROLE_INSTRUCTIONS[stage], agent.prompt].filter(Boolean).join('\n\n') }, ...toolPresetMessages(presetFor(stage)), ...extras.filter(message => message.cacheStatic !== false && staticText(message.content)).map(({ role, content }) => ({ role, content })), { role: 'user', content: JSON.stringify(input) }, ...extras.filter(message => message.cacheStatic === false || !staticText(message.content)).map(({ role, content }) => ({ role, content }))];
    ...
    validate?.(result);
    const record = { stage, label, provider: agent.provider, model: agent.model, elapsedMs: Date.now() - startedAt, usage, result };
    trace.push(record);
  ...
  try {
    ...
    recalls = reuse ? reuse.recalls : await mapConcurrent(actors, config.concurrency, async character => {
      const memories = state.memories[character.id] ?? [];
    ...
    if (reuse) {
      ...
      combined = await stageCall('combine', {
        ...
        worldbookDirectory: worldbook.filter(e => e.enabled && e.content.trim()).map(e => ({ id: e.id, title: e.comment, keys: e.keys })),
        activeWorldbook: stableFirst(activated.entries).map(e => ({ id: e.id, title: e.comment, content: e.content })),
      ...
      plan = await stageCall('advance', { worldbook: stableFirst(worldbook.filter(e => e.enabled && combined.worldEntryIds.includes(e.id))).map(e => ({ id: e.id, content: e.content })), ...publicScene, combination: combined, recalls: recalls.map(recallView), worldState: worldState.map(fact => factView(fact)), worldEpisodes: worldEpisodes.map(episode => episodeView(episode)), tables: currentTables, summaryRecords: advance?.summaryRecords ?? [], ...(advance ? { advanceTasks: advance.results } : {}) }, PLAN, '剧情推进', advance ? [] : legacyPromptMessages(legacy, 'advance', replacements), undefined, plan => assertIds(plan.characterIntents.map(c => c.characterId), combined.presentCharacterIds, '推进计划角色'));
    }
    emit({ type: 'plan', combination: combined, plan });
    }
    const writingPreset = presetFor('write');
    const writing = await assembleWritingPrompt({ state, card, preset: writingPreset, worldbook, plan, summarySelectionEnabled, regex, extraContext: normal ? '' : `本轮场景与角色视角：\n${JSON.stringify(combined)}\n召回资料（保留角色归属及经历时间）：\n${JSON.stringify(recalls.map(recallView))}\n最后确认的世界状态：\n${JSON.stringify(worldState.map(fact => factView(fact)))}\n相关世界经历：\n${JSON.stringify(worldEpisodes.map(episode => episodeView(episode)))}\n当前状态优先于较早的经历；角色知识以各自 currentState 和经历为依据。`, trigger, signal: runSignal, scanWorldbook, deferBudget: true });
    ...
    const usage = [], startedAt = Date.now();
    let reasoning = '';
    let story = await callModel({ stage: 'write', label: '写作', agent: writer, messages: writing.messages, signal: runSignal, onText: chunk => emit({ type: 'text', text: chunk }), onReasoning: value => { reasoning = value; }, onUsage: value => usage.push(value), retries: 0 });
    if (!/\[角色\|/.test(story) && /(?:^|\n)[^\[\n]{1,40}\|[^|\n]{1,16}\|\[/m.test(story)) {
    ...
    trace.push({ stage: 'write', label: '写作', provider: writer.provider, model: writer.model, elapsedMs: Date.now() - startedAt, usage, worldEntryIds: writing.worldEntryIds, request: writing.messages });
    ...
    const assistant = { id: randomUUID(), role: 'assistant', name: card.name, content: post.texts[0], ...(reasoning ? { extra: { reasoning } } : {}), createdAt: new Date().toISOString(), turnId };
    assistant.content = await repairInteraction(assistant.content, async (content, error) => {
      const usage = [], startedAt = Date.now(), label = '修正交互数据';
      const result = await callModel({ stage: 'format', label, agent: writer, schema: INTERACTION, retries: config.protocolRetries, signal: runSignal,
      ...
      trace.push({ stage: 'format', label, provider: writer.provider, model: writer.model, elapsedMs: Date.now() - startedAt, usage, result });
      return result;
    });
    if (afterWrite) {
    ...
    const updateMemory = async character => {
      ...
      const previousMemories = [...new Map([...recall.records, ...(state.memories[character.id] ?? []).slice(-config.recallCount)].map(memory => [memory.id, memory])).values()];
      const check = memory => {
      ...
      const memory = await stageCall('memory', { character, previousMemories: previousMemories.map(memory => episodeView(memory)), currentState: currentState.map(fact => factView(fact, true)), previousScene: state.scene, userInput: text, completedStory: assistant.content, sourcePassages }, MEMORY, character.name, [], character.memoryModel, check);
    ...
    const updates = await mapConcurrent([
      ...memoryActors.map(character => ({ type: 'memory', character })),
      { type: 'table' },
      ...
      const tables = await stageCall('table', {
        ...
        previousScene: state.scene, userInput: text, completedStory: assistant.content, completedStoryMessageId: assistant.id,
    ...
    const newCharacters = tableUpdate.newCharacters.map(character => ({ ...character, id: randomUUID(), enabled: true, worldbookIds: [] }));
    ...
    updates.push(...await mapConcurrent([...additionalActors, ...newCharacters], config.concurrency, updateMemory));
    ...
    for (const update of updates) {
      if (update.type === 'table') {
        ...
        if (memory.summary || memory.facts.length || memory.relationships.length || memory.openThreads.length || memory.stateChanges.length) {
