import { memoryEpisodes } from './memory-view.js';
  ...
  return function TavernSettings({ useSessions, sessionId: boundSessionId, embedded = false, initialPage = 'resources', scope = 'session' }) {
    ...
    return <section className="tavern-settings">
      ...
      {page === 'agents' && <>
        ...
        <div className="tavern-grid"><label>正文目标字数下限<input aria-label="正文目标字数下限" type="number" min="1" value={config.writingMinChars ?? DEFAULT_WRITING_LENGTH.min} onChange={event => setConfig({ ...config, writingMinChars: Number(event.target.value) })} /></label><label>正文目标字数上限<input aria-label="正文目标字数上限" type="number" min="1" value={config.writingMaxChars ?? DEFAULT_WRITING_LENGTH.max} onChange={event => setConfig({ ...config, writingMaxChars: Number(event.target.value) })} /></label></div>
        <p className="tavern-caption">字数区间仅写进正文提示词，不计思考、HTML 与协议数据。超过目标也不强行截断；持续输出正文、思考或工具参数时会继续等待。</p>
        ...
        {config.playMode !== 'normal' && <div className="tavern-grid"><label>并发数<input type="number" min="1" max="16" value={config.concurrency} onChange={event => setConfig({ ...config, concurrency: Number(event.target.value) })} /></label><label>近期对话轮数<input type="number" min="1" value={config.historyTurns} onChange={event => setConfig({ ...config, historyTurns: Number(event.target.value) })} /></label><label>自动纠错重试次数<input type="number" min="0" max="3" value={config.protocolRetries} onChange={event => setConfig({ ...config, protocolRetries: Number(event.target.value) })} /><span className="tavern-caption">每个工具或推进任务的结果校验失败后，最多额外纠错的次数。3 表示首次请求后最多再纠错 3 次，成功即停止。拒答、无可修复结果、输出截断、静默超时或取消时停止；网络重试遵循提供方设置。</span></label></div>}
