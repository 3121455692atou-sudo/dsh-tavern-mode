...
ROLE_INSTRUCTIONS.table += ' participatingCharacters 列出 characters 中 enabled 为 true 且在本轮已完成正文中实际经历、观察或明确获知事件的角色，每个 characterId 只出现一次，包括本轮首次参与的已有角色；仅被提及、出现在计划中或未获知事件的角色不列入，无参与者时返回空数组。每项 evidence.sourceId 必须引用 sourcePassages 中 messageId 等于 completedStoryMessageId 且支持该角色实际参与的原文段落。newCharacters 仅补录本轮正文首次实际出场的具名非玩家角色，并提供支持其身份与基础设定的正文原文依据；已经在 characters 中的角色沿用原身份，不重复创建，未出场的计划角色不补录。';
