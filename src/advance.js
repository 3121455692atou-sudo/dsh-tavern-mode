...
export async function runAdvancePreset({ state, card, data, worldbook, advanceWorldbooks, text, callModel, signal, emit, trace, mapConcurrent }) {
  const profile = data?.plotSettings ?? data;
  ...
  for (const task of tasks) {
    ...
    const summaryRecords = selectedRecords();
      ...
      const values = { ...replacements, '$1': stableFirst(active).map(entry => entry.content).join('\n\n'), '$7': contextFor(options) };
  ...
  return { results, injection, summaryRecords: selectedRecords() };
}
