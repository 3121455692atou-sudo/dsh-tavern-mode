window.__ModuleLoader__.load({ id: 'dsh-tavern-mode', factory: require => { const module = { exports: {} }; const exports = module.exports;
...
var require_unescapeHtmlChar = __commonJS({
  "node_modules/lodash/_unescapeHtmlChar.js"(exports, module2) {
...
var require_unescape = __commonJS({
  "node_modules/lodash/unescape.js"(exports, module2) {
    var toString = require_toString();
    ...
    module2.exports = unescape2;
  }
});

// src/native-client.jsx
var native_client_exports = {};
__export(native_client_exports, {
  apply: () => apply,
  inject: () => inject
});
module.exports = __toCommonJS(native_client_exports);
var import_react7 = __toESM(require("react"), 1);
var import_dsh_client_ui_primitives = require("@deepseek-ai/dsh-client-ui-primitives");

// src/macros.js
var import_get = __toESM(require_get(), 1);
var import_unescape = __toESM(require_unescape(), 1);
...
// src/template-compat/seedrandom.js
var seedrandom = (function(global2, pool, math) {
...
function pickMacro(rawContent, { sessionId, chatMetadata = {} }) {
  ...
  function getPickReplaceMacro(rawContent2) {
    const chatIdHash = getChatIdHash();
    const rawContentHash = getStringHash(rawContent2);
    ...
    const pickReplace = (match, listString, offset) => {
      ...
      const rng = seedrandom_default(finalSeed);
...
// src/template-compat/regex.js
function regexDispatch(rawString, scripts, placement, params, runRegexScript) {
  const extension_settings = { disabledExtensions: [] };
  const getRegexScripts = () => scripts;
  const console2 = { debug() {
  }, warn: (...args) => globalThis.console.warn(...args) };
  function getRegexedString(rawString2, placement2, { characterOverride, isMarkdown, isPrompt, isEdit, depth } = {}) {
    if (typeof rawString2 !== "string") {
      console2.warn("getRegexedString: rawString is not a string. Returning empty string.");
      return "";
    }
    let finalString = rawString2;
    if (extension_settings.disabledExtensions.includes("regex") || !rawString2 || placement2 === void 0) {
      return finalString;
    }
    ...
    allRegex.forEach((script) => {
      if (
        ...
        script.markdownOnly && isMarkdown || // Script applies to Generate and input is Generate
        script.promptOnly && isPrompt || // Script applies to all cases when neither "only"s are true, but there's no need to do it when `isMarkdown`, the as source (chat history) should already be changed beforehand
        !script.markdownOnly && !script.promptOnly && !isMarkdown && !isPrompt
      ) {
        if (isEdit && !script.runOnEdit) {
          console2.debug(`getRegexedString: Skipping script ${script.scriptName} because it does not run on edit`);
          return;
        }
        if (typeof depth === "number") {
          if (!isNaN(script.minDepth) && script.minDepth !== null && script.minDepth >= -1 && depth < script.minDepth) {
            console2.debug(`getRegexedString: Skipping script ${script.scriptName} because depth ${depth} is less than minDepth ${script.minDepth}`);
            return;
          }
          if (!isNaN(script.maxDepth) && script.maxDepth !== null && script.maxDepth >= 0 && depth > script.maxDepth) {
            console2.debug(`getRegexedString: Skipping script ${script.scriptName} because depth ${depth} is greater than maxDepth ${script.maxDepth}`);
            return;
          }
        }
        if (script.placement.includes(placement2)) {
...
// src/macros.js
function macroEnvironment(input = {}) {
  return { variables: {}, globalVariables: {}, messages: [], user: "\u4F60", char: "", now: (/* @__PURE__ */ new Date()).toISOString(), diagnostics: [], ...input };
}
...
function variableValue(env, scope, path) {
  ...
  if (scope === "message") {
    const index = env.messageId ?? Object.keys(env.messageVariables ?? {}).map(Number).sort((a, b2) => b2 - a).find((index2) => env.messageVariables[index2] !== void 0);
  ...
  return withoutMetadata((0, import_get.default)(variables, (0, import_unescape.default)(path), null));
}
function expandMacros(source, env, transform = (value) => value) {
  ...
  for (let pass = 0; pass < 40; pass++) {
    ...
    result = result.replace(/\{\{([^{}]*?)\}\}/g, (whole, body, offset) => {
      ...
      if (name.startsWith("//")) replacement = "";
        ...
        replacement = typeof variable === "string" ? variable : kind === "format" ? stringify3(variable, { blockQuote: "literal" }).trimEnd() : JSON.stringify(variable);
        if (kind === "format") replacement = replacement.replaceAll("\n", "\n" + " ".repeat(offset - result.lastIndexOf("\n", offset - 1) - 1));
      } else if (name === "setvar" || name === "setglobalvar") {
        (name === "setvar" ? env.variables : env.globalVariables)[key] = value;
        replacement = "";
      ...
      } else if (name === "unsetvar" || name === "unsetglobalvar") {
        delete (name === "unsetvar" ? env.variables : env.globalVariables)[key];
        replacement = "";
        ...
        if (Number(n || 1) > 1e4 || Number(sides) < 1) throw new Error("\u63B7\u9AB0\u5B8F\u53C2\u6570\u8D85\u51FA\u8303\u56F4");
        replacement = String(Array.from({ length: Number(n || 1) }, () => 1 + Math.floor((env.random ?? Math.random)() * Number(sides))).reduce((a, b2) => a + b2, Number(plus || 0)));
      } else {
        ...
        const values = {
          ...
          lastusermessage: messages.findLast((m2) => m2.role === "user")?.content ?? "",
          lastcharmessage: messages.findLast((m2) => m2.role === "assistant")?.content ?? "",
          lastmessage: messages.at(-1)?.content ?? "",
          lastmessageid: String(Math.max(0, messages.length - 1)),
          chatid: env.sessionId ?? "",
          date: new Date(env.now).toLocaleDateString("zh-CN"),
          time: new Date(env.now).toLocaleTimeString("zh-CN", { hour12: false }),
          weekday: new Date(env.now).toLocaleDateString("zh-CN", { weekday: "long" }),
          isodate: env.now.slice(0, 10),
          isotime: env.now.slice(11, 19),
          newline: "\n",
          noop: "",
          input: env.input ?? ""
        };
        if (Object.hasOwn(values, name)) replacement = stored(values[name]);
          ...
          if (!env.diagnostics.includes(`\u672A\u8BC6\u522B\u5B8F\uFF1A${name}`)) env.diagnostics.push(`\u672A\u8BC6\u522B\u5B8F\uFF1A${name}`);
          return whole;
        }
      }
...
function applyRegex(text, scripts, { placement = 2, phase = "display", depth, edit = false, env = macroEnvironment() } = {}) {
  return regexDispatch(text, (scripts ?? []).filter((script) => script.placement?.includes(placement)), placement, { isMarkdown: phase === "display", isPrompt: phase === "prompt", isEdit: edit, depth }, (script, text2) => {
    if (script.disabled || !script.findRegex) return text2;
    ...
    try {
      ...
      throw new Error(`\u6B63\u5219 ${script.scriptName ?? script.id} \u65E0\u6548\uFF1A${error.message}`);
    }
    return text2.replace(regex, (...args) => {
      ...
      replacement = replacement.replace(/\$(\d+)|\$<([^>]+)>/g, (_2, number, name) => {
        let value = number !== void 0 ? Number(number) < count ? args[Number(number)] ?? "" : "" : groups[name] ?? "";
...
async function api(path, value) {
  const response = await fetch("/api/tavern" + path, value === void 0 ? void 0 : { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(value) });
  const result = await response.json();
  if (!response.ok || result.error) throw new Error(result.error ?? `\u8BF7\u6C42\u5931\u8D25\uFF1A${response.status}`);
  return result;
}
function createBrowserRuntime(ctx) {
  const controllers = /* @__PURE__ */ new Map(), snapshots = /* @__PURE__ */ new Map(), listeners = /* @__PURE__ */ new Map(), refreshes = /* @__PURE__ */ new Map();
  ...
  async function loadBootstrap(refresh2 = false) {
    if (!bootstrap || refresh2) bootstrap = await api("/bootstrap");
    return bootstrap;
  }
  function publish(id, value) {
    snapshots.set(id, { ...snapshots.get(id), ...value });
    for (const callback of listeners.get(id) ?? []) callback();
  }
  function input(id) {
    const scope = ctx.sessions.scope(id);
    return scope && ctx.conversation.input.for(scope);
  }
  function report(id, message) {
    publish(id, { error: message });
    input(id)?.notify("error", message);
  }
  ...
  async function refreshNow(id, remount = false) {
    ...
    if (!payload.state) {
      publish(id, { payload: null, draftConfig: payload.config, error: null, loaded: true });
      return payload;
    }
  ...
  async function ensure(id) {
    ...
    const controller = { id, hubId: crypto.randomUUID(), pending: /* @__PURE__ */ new Map() };
    ...
    controller.ready = (async () => {
      ...
      const timeout = setTimeout(() => rejectReady(new Error("\u89D2\u8272\u524D\u7AEF\u52A0\u8F7D\u8D85\u65F6")), 25e3);
      ...
      controller.call = (method, args) => new Promise((resolve, reject) => {
        ...
        const timeoutMs = ["button", "settings"].includes(method) ? 0 : method === "messageReceived" ? 66e4 : 2e4;
        const timer = timeoutMs ? setTimeout(() => {
          controller.pending.delete(key);
          reject(new Error(`\u524D\u7AEF ${method} \u54CD\u5E94\u8D85\u65F6`));
        }, timeoutMs) : void 0;
      ...
      controller.port.onmessage = async (event) => {
        ...
        if (data.type === "ready") {
          ...
          controller.rectangles = data.rectangles;
          layoutOverlays();
        } else if (data.type === "buttons") {
          controller.buttons = data.buttons;
          publish(id, { payload: snapshots.get(id)?.payload });
        } else if (data.type === "regex") {
          publish(id, { payload: { ...snapshots.get(id)?.payload, regex: data.regex } });
        } else if (data.type === "result") {
          const request = controller.pending.get(data.id);
          if (!request) return;
          controller.pending.delete(data.id);
          ...
          try {
            ...
            controller.port.postMessage({ type: "rpc-result", id: data.id, error: error.message });
          }
        } else if (data.type === "draft") input(id)?.setDraft(data.text);
        else if (data.type === "send") {
          const target = input(id);
          if (target) {
            target.setDraft(data.text || "\u7EE7\u7EED\u3002");
            target.submit();
          }
      ...
      const onEvent = async (event) => {
        ...
        try {
          if (data.type === "runtime") {
            try {
              const value = await controller.call(data.method, data.args);
              await api("/native-reply", { sessionId: id, id: data.id, value });
            } catch (error) {
              await api("/native-reply", { sessionId: id, id: data.id, error: error.message });
            }
          } else if (data.type === "assets") {
            controller.assetsStale = true;
            if (!snapshots.get(id)?.payload?.generating) {
            ...
            await refresh(id);
            await controller.call("finished", { cancelled: !!data.cancelled });
            if (controller.assetsStale) {
              await controller.call("flush", {});
              await refresh(id, true);
            }
          } else if (data.type === "error") report(id, data.message);
      ...
      const connectEvents = () => {
        ...
        controller.events.onclose = (event) => {
          if (!controller.disposed && event.code !== 1008) controller.reconnect = setTimeout(connectEvents, 1e3);
        };
      };
      connectEvents();
      ...
      if (!controller.disposed) report(id, error.message);
      controller.dispose();
    ...
    controller.dispose = () => {
      ...
      for (const request of controller.pending.values()) {
        clearTimeout(request.timer);
        request.reject(new Error("\u89D2\u8272\u524D\u7AEF\u5DF2\u5173\u95ED"));
      }
      controller.pending.clear();
      if (controllers.get(id) === controller) publish(id, { ready: false, runtimeId: void 0 });
    };
    return controller.ready;
  }
  const updateTheme = () => {
    const value = theme();
    for (const controller of controllers.values()) controller.call?.("theme", value).catch(() => {
    });
  };
  ...
  return {
    ...
    mountView(id, element) {
      ...
      view = element ? { id, element } : void 0;
      viewportObserver.disconnect();
      if (element) viewportObserver.observe(element);
      layoutOverlays();
    },
    clearNotice(id, noticeId) {
      if (snapshots.get(id)?.notice?.id === noticeId) publish(id, { notice: null });
    },
    ...
    retryUpdates(id) {
      ...
      target.setDraft("/tavern-retry-updates");
      target.submit();
    },
...
// src/native-component-binding.jsx
var import_react = __toESM(require("react"), 1);
var bindingKey = Symbol.for("dsh-tavern-mode/component-binding");
function bindComponent(entry, wrap) {
  let binding = entry.component[bindingKey];
  if (!binding) {
    let ComponentBinding = function(props) {
      const Component = (0, import_react.useSyncExternalStore)(binding.subscribe, binding.snapshot);
      return /* @__PURE__ */ import_react.default.createElement(Component, { ...props });
    };
    binding = { current: entry.component, listeners: /* @__PURE__ */ new Set() };
    binding.subscribe = (callback) => {
      binding.listeners.add(callback);
      return () => binding.listeners.delete(callback);
    };
  ...
  const publish = (component) => {
    binding.current = component;
    for (const callback of binding.listeners) callback();
  };
  publish(current);
  return () => {
    if (binding.current === current) publish(previous);
  };
}

// src/memory-view.js
function memoryEpisodes(state, characterId) {
...
// src/native-settings.jsx
var import_react4 = __toESM(require("react"), 1);

// vendor/default-preset.json
var default_preset_default = {
...
// src/presets.js
var { openai_max_tokens: _defaultOutputLimit, openai_max_context: _defaultContextLimit, ...defaultWritingPreset } = default_preset_default;
function presetPrompts(preset, characterId = 100001) {
  if (!preset) return [];
  if (preset.settings && !preset.prompt_order) return preset.prompts ?? [];
  const order = (preset.prompt_order ?? []).find((x2) => String(x2.character_id) === String(characterId)) ?? preset.prompt_order?.find((x2) => x2.character_id === 100001) ?? preset.prompt_order?.[0];
...
// src/memory-state.js
var factKey = (fact) => JSON.stringify([fact.subject, fact.key]);
function currentFacts(episodes) {
...
// src/native-resources.jsx
var import_react2 = __toESM(require("react"), 1);
function ResourceImporter({ imported }) {
  const input = (0, import_react2.useRef)();
  const [busy, setBusy] = (0, import_react2.useState)(false), [dragging, setDragging] = (0, import_react2.useState)(false), [paths, setPaths] = (0, import_react2.useState)("");
  const [message, setMessage] = (0, import_react2.useState)(""), [errors, setErrors] = (0, import_react2.useState)([]);
    ...
    try {
      for (const source of sources) {
        try {
          ...
          failures.push(`${typeof source === "string" ? source : source.name}\uFF1A${error.message}`);
        }
      }
      if (items.length) {
        await imported(items);
        setMessage(`\u5DF2\u5BFC\u5165 ${items.length} \u9879\u8D44\u6E90`);
      }
    } catch (error) {
  ...
  return /* @__PURE__ */ import_react2.default.createElement("div", { className: "tavern-import" }, /* @__PURE__ */ import_react2.default.createElement("input", { ref: input, type: "file", "aria-label": "\u5BFC\u5165\u89D2\u8272\u5361\u3001\u9884\u8BBE\u6216\u8D44\u6E90\u5305", accept: ".json,.png,.zip", multiple: true, hidden: true, disabled: busy, onChange: (event) => {
    const files = [...event.target.files];
    event.target.value = "";
    if (files.length) importResources(files);
  } }), /* @__PURE__ */ import_react2.default.createElement(
    "button",
    {
      type: "button",
      className: "tavern-import-drop",
      ...
      onDrop: (event) => {
        ...
        if (!busy && event.dataTransfer.files.length) importResources([...event.dataTransfer.files]);
      }
    },
    /* @__PURE__ */ import_react2.default.createElement("span", { className: "tavern-import-plus", "aria-hidden": "true" }, "\uFF0B"),
    /* @__PURE__ */ import_react2.default.createElement("strong", null, busy ? "\u6B63\u5728\u5BFC\u5165\u2026" : "\u5BFC\u5165\u89D2\u8272\u5361\u3001\u9884\u8BBE\u6216\u8D44\u6E90\u5305"),
    /* @__PURE__ */ import_react2.default.createElement("span", { className: "tavern-caption" }, "\u70B9\u51FB\u9009\u62E9\u6216\u62D6\u5165\u6587\u4EF6 \xB7 PNG / JSON / ZIP")
  ), /* @__PURE__ */ import_react2.default.createElement("details", { className: "tavern-import-path" }, /* @__PURE__ */ import_react2.default.createElement("summary", null, "\u4ECE\u672C\u673A\u8DEF\u5F84\u5BFC\u5165"), /* @__PURE__ */ import_react2.default.createElement("label", null, "\u672C\u673A\u6587\u4EF6\u8DEF\u5F84", /* @__PURE__ */ import_react2.default.createElement("textarea", { rows: "2", placeholder: "\u6BCF\u884C\u4E00\u4E2A\u5B8C\u6574\u8DEF\u5F84", value: paths, onChange: (event) => setPaths(event.target.value) })), /* @__PURE__ */ import_react2.default.createElement("button", { type: "button", disabled: busy || !paths.trim(), onClick: () => importResources(paths.split("\n").map((path) => path.trim()).filter(Boolean)) }, "\u5BFC\u5165\u6587\u4EF6")), message && /* @__PURE__ */ import_react2.default.createElement("p", { role: "status" }, message), errors.map((error, index) => /* @__PURE__ */ import_react2.default.createElement("p", { className: "tavern-error", role: "alert", key: index }, error)));
}
function ResourceLibrary({ runtime, sessionId, selected }) {
  const [data, setData] = (0, import_react2.useState)(), [query, setQuery] = (0, import_react2.useState)(""), [busy, setBusy] = (0, import_react2.useState)(""), [error, setError] = (0, import_react2.useState)("");
  const load = async () => {
  ...
  (0, import_react2.useEffect)(() => {
    load().catch((error2) => setError(error2.message));
  }, [sessionId]);
...
${item.sourceName}`.toLocaleLowerCase().includes(query.toLocaleLowerCase()))) ?? [];
  return /* @__PURE__ */ import_react2.default.createElement("section", { className: "tavern-settings tavern-library" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tavern-library-title" }, /* @__PURE__ */ import_react2.default.createElement("div", null, /* @__PURE__ */ import_react2.default.createElement("h2", null, "\u9009\u62E9\u89D2\u8272\u5361"), /* @__PURE__ */ import_react2.default.createElement("p", { className: "tavern-caption" }, "\u5BFC\u5165\u89D2\u8272\u5361\u540E\uFF0C\u9009\u62E9\u4E00\u5F20\u5F00\u59CB\u4F1A\u8BDD\u3002"))), /* @__PURE__ */ import_react2.default.createElement(ResourceImporter, { imported: load }), error && /* @__PURE__ */ import_react2.default.createElement("p", { className: "tavern-error", role: "alert" }, error), data?.library.some((item) => item.kind === "card") && /* @__PURE__ */ import_react2.default.createElement("input", { className: "tavern-card-search", type: "search", "aria-label": "\u67E5\u627E\u89D2\u8272\u5361", placeholder: "\u67E5\u627E\u89D2\u8272\u5361", value: query, onChange: (event) => setQuery(event.target.value) }), !data && !error && /* @__PURE__ */ import_react2.default.createElement("p", { className: "tavern-caption" }, "\u6B63\u5728\u8BFB\u53D6\u89D2\u8272\u5361\u2026"), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tavern-card-grid" }, cards.map((card) => /* @__PURE__ */ import_react2.default.createElement("button", { type: "button", className: "tavern-card", key: card.id, disabled: !!busy, onClick: () => choose(card) }, card.cover ? /* @__PURE__ */ import_react2.default.createElement("img", { src: "/api/tavern/asset?id=" + encodeURIComponent(card.cover), alt: "" }) : /* @__PURE__ */ import_react2.default.createElement("span", { className: "tavern-card-cover", "aria-hidden": "true" }, card.name[0]), /* @__PURE__ */ import_react2.default.createElement("span", { className: "tavern-card-info" }, /* @__PURE__ */ import_react2.default.createElement("strong", null, busy === card.id ? "\u6B63\u5728\u6253\u5F00\u2026" : card.name), /* @__PURE__ */ import_react2.default.createElement("small", null, card.sourceName))))));
}

// src/native-messages.jsx
var import_react3 = __toESM(require("react"), 1);

// src/native-display.js
function findMessageIndex(state, nativeId, text, role) {
...
function displayMessage(state, message, fallback = "") {
  const helper = message?.id ? state.helperChat?.find((item) => item.tavernMessageId === message.id) : void 0;
  return { text: helper?.mes ?? message?.content ?? fallback, hidden: helper?.is_system ?? message?.is_hidden ?? false };
}

// src/native-messages.jsx
function createMessageActions(runtime) {
  return function MessageActions({ sessionId, payload, message, nativeMessageId, text, role, children, changed, reasoning = "" }) {
    const [editing, setEditing] = (0, import_react3.useState)(false), [draft, setDraft] = (0, import_react3.useState)("");
    const [busy, setBusy] = (0, import_react3.useState)(false), [error, setError] = (0, import_react3.useState)("");
    const [copied, setCopied] = (0, import_react3.useState)(false), reasoningRef = (0, import_react3.useRef)();
    ...
    async function act(action, text2) {
      setBusy(true);
      setError("");
      try {
        await runtime.messageAction(sessionId, { action, messageId: message?.id, nativeMessageId, ...text2 === void 0 ? {} : { text: text2 } });
        await changed?.();
        setEditing(false);
      } catch (error2) {
        setError(error2.message);
    ...
    return /* @__PURE__ */ import_react3.default.createElement("div", { className: "tavern-message", "data-message-role": role }, message?.id && message.id === payload.state.pendingUpdates?.context?.assistant?.id && /* @__PURE__ */ import_react3.default.createElement("div", { className: "tavern-error", role: "status" }, "\u6B63\u6587\u5DF2\u4FDD\u5B58\u3002\u8868\u683C\u6216\u8BB0\u5FC6\u66F4\u65B0", payload.generating ? "\u8FDB\u884C\u4E2D" : "\u672A\u5B8C\u6210", "\u3002", payload.state.pendingUpdates.error && /* @__PURE__ */ import_react3.default.createElement("span", null, payload.state.pendingUpdates.error), !payload.generating && /* @__PURE__ */ import_react3.default.createElement("button", { disabled: busy, onClick: () => runtime.retryUpdates(sessionId) }, "\u4EC5\u91CD\u8BD5\u672A\u5B8C\u6210\u66F4\u65B0")), role === "assistant" && thinking && /* @__PURE__ */ import_react3.default.createElement("details", { className: "tavern-thinking" }, /* @__PURE__ */ import_react3.default.createElement("summary", null, "\u601D\u8003\u8FC7\u7A0B \xB7 ", Array.from(thinking).length.toLocaleString(), " \u5B57\u7B26"), /* @__PURE__ */ import_react3.default.createElement("div", { className: "tavern-message-actions" }, /* @__PURE__ */ import_react3.default.createElement("button", { onClick: async () => {
      try {
        await navigator.clipboard.writeText(thinking);
        setCopied(true);
      } catch {
        reasoningRef.current?.focus();
        reasoningRef.current?.select();
        setError("\u5DF2\u9009\u4E2D\u601D\u8003\u539F\u6587\uFF0C\u8BF7\u624B\u52A8\u590D\u5236\u3002");
      }
    } }, copied ? "\u5DF2\u590D\u5236\u5168\u90E8\u601D\u8003" : "\u590D\u5236\u5168\u90E8\u601D\u8003")), /* @__PURE__ */ import_react3.default.createElement("textarea", { ref: reasoningRef, className: "tavern-message-editor tavern-thinking-text", "aria-label": "\u601D\u8003\u539F\u6587\uFF0C\u53EF\u9009\u62E9\u590D\u5236", value: thinking, readOnly: true, spellCheck: false })), editing ? /* @__PURE__ */ import_react3.default.createElement("textarea", { className: "tavern-message-editor", "aria-label": role === "user" ? "\u7F16\u8F91\u7528\u6237\u6D88\u606F" : "\u7F16\u8F91\u6A21\u578B\u6D88\u606F", value: draft, onChange: (event) => setDraft(event.target.value), autoFocus: true }) : children, /* @__PURE__ */ import_react3.default.createElement("div", { className: "tavern-message-actions" }, editing ? /* @__PURE__ */ import_react3.default.createElement(import_react3.default.Fragment, null, /* @__PURE__ */ import_react3.default.createElement("button", { disabled, onClick: () => act("edit", draft) }, "\u4FDD\u5B58"), canReroll && role === "user" && /* @__PURE__ */ import_react3.default.createElement("button", { disabled: disabled || !draft.trim(), onClick: () => act("reroll", draft) }, "\u4FDD\u5B58\u5E76\u91CD\u65B0\u751F\u6210"), /* @__PURE__ */ import_react3.default.createElement("button", { disabled: busy, onClick: () => setEditing(false) }, "\u53D6\u6D88")) : /* @__PURE__ */ import_react3.default.createElement(import_react3.default.Fragment, null, /* @__PURE__ */ import_react3.default.createElement("button", { disabled, onClick: () => {
      setDraft(content);
      setEditing(true);
    } }, "\u7F16\u8F91"), canReroll && /* @__PURE__ */ import_react3.default.createElement("button", { disabled, onClick: () => act("reroll") }, role === "user" ? "\u4ECE\u8FD9\u91CC\u91CD\u65B0\u751F\u6210" : "\u5168\u90E8\u91CD\u65B0\u751F\u6210"), canReroll && role === "assistant" && /* @__PURE__ */ import_react3.default.createElement("button", { disabled, onClick: () => act("rewrite") }, "\u91CD\u5199\u6B63\u6587"), /* @__PURE__ */ import_react3.default.createElement("button", { disabled, onClick: () => act("delete") }, "\u5220\u9664"))), error && /* @__PURE__ */ import_react3.default.createElement("p", { className: "tavern-error", role: "alert" }, error));
  };
}

// src/writing-length.js
var DEFAULT_WRITING_LENGTH = Object.freeze({ min: 2e3, max: 4e3 });

// src/native-settings.jsx
var labels = { recall: "\u53EC\u56DE", combine: "\u7EC4\u5408", advance: "\u63A8\u8FDB", write: "\u5199\u4F5C", memory: "\u8BB0\u5FC6", table: "\u586B\u8868" };
var pages = { resources: "\u89D2\u8272\u5361\u4E0E\u9884\u8BBE", messages: "\u5386\u53F2\u6D88\u606F", agents: "\u6A21\u578B\u4E0E\u8F93\u5165", memory: "\u89D2\u8272\u8BB0\u5FC6", tables: "\u8868\u683C", frontend: "\u524D\u7AEF\u4E0E\u53D8\u91CF" };
var globalPages = { resources: "\u89D2\u8272\u5361\u4E0E\u9884\u8BBE", agents: "\u6A21\u578B\u4E0E\u8F93\u5165", frontend: "\u663E\u793A\u65B9\u5F0F" };
var clone = (value) => structuredClone(value);
var selectedFields = ["cardId", "presetId", "toolPresetId", "tableId", "legacyId", "bubbleId", "renderMode", "userName", "persona", "worldbookIds", "regexIds"];
var fromState = (state) => Object.fromEntries(selectedFields.map((key) => [key, state[key]]));
function StateFacts({ episodes, title }) {
  const facts = currentFacts(episodes);
  if (!facts.length) return null;
  return /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, title), facts.map((fact) => /* @__PURE__ */ import_react4.default.createElement("details", { key: fact.id }, /* @__PURE__ */ import_react4.default.createElement("summary", null, fact.subject, " \xB7 ", fact.key, "\uFF1A", fact.value), /* @__PURE__ */ import_react4.default.createElement("p", null, fact.evidence.quote), /* @__PURE__ */ import_react4.default.createElement("small", null, new Date(fact.createdAt).toLocaleString()))));
}
function ModelAttempts({ sessionId }) {
  const [records, setRecords] = (0, import_react4.useState)([]), [error, setError] = (0, import_react4.useState)(""), [loaded, setLoaded] = (0, import_react4.useState)(false);
  return /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u5931\u8D25\u4E0E\u91CD\u8BD5\u8BB0\u5F55"), /* @__PURE__ */ import_react4.default.createElement("button", { onClick: async () => {
    try {
      setRecords(await api("/model-attempts?id=" + sessionId));
      setLoaded(true);
      setError("");
    } catch (error2) {
      setError(error2.message);
    }
  } }, "\u8BFB\u53D6\u8BB0\u5F55"), error && /* @__PURE__ */ import_react4.default.createElement("p", { role: "alert" }, error), loaded && !records.length && /* @__PURE__ */ import_react4.default.createElement("p", null, "\u6682\u65E0\u8BB0\u5F55"), records.map((record) => /* @__PURE__ */ import_react4.default.createElement("details", { key: record.taskId + ":" + record.attempt }, /* @__PURE__ */ import_react4.default.createElement("summary", null, new Date(record.createdAt).toLocaleString(), " \xB7 ", record.label, " \xB7 \u7B2C ", record.attempt, " \u6B21\u5C1D\u8BD5 \xB7 ", { retrying: "\u5C06\u91CD\u8BD5", failed: "\u5931\u8D25", succeeded: "\u6210\u529F" }[record.status]), record.error && /* @__PURE__ */ import_react4.default.createElement("p", null, record.error.message), /* @__PURE__ */ import_react4.default.createElement("pre", null, JSON.stringify(record.response, null, 2)))));
}
function ResourceSelect({ label, kind, value, change, library, required, children }) {
  return /* @__PURE__ */ import_react4.default.createElement("label", null, label, /* @__PURE__ */ import_react4.default.createElement("select", { value: value || "", onChange: (event) => change(event.target.value) }, /* @__PURE__ */ import_react4.default.createElement("option", { value: "" }, required ? "\u9009\u62E9\u89D2\u8272\u5361" : children || "\u5185\u7F6E\u9ED8\u8BA4"), library.filter((item) => item.kind === kind).map((item) => /* @__PURE__ */ import_react4.default.createElement("option", { key: item.id, value: item.id }, item.name))));
}
function ModelFields({ value, change, providers, name, sessionId, compact = false }) {
  const [info, setInfo] = (0, import_react4.useState)(), [error, setError] = (0, import_react4.useState)("");
  (0, import_react4.useEffect)(() => {
    let active = true;
    setInfo(void 0);
    setError("");
    api("/model-info?" + new URLSearchParams({ sessionId: sessionId ?? "", provider: value.provider ?? "", model: value.model ?? "" })).then((info2) => {
      if (active) setInfo(info2);
    }).catch((error2) => {
      if (active) setError(error2.message);
    });
    return () => {
      active = false;
    };
  ...
  const set2 = (key, next) => change({ ...value, [key]: next });
  return /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-model" }, !compact && /* @__PURE__ */ import_react4.default.createElement("label", null, "\u63A8\u7406\u5F3A\u5EA6", /* @__PURE__ */ import_react4.default.createElement("select", { "aria-label": `${name}\u63A8\u7406\u5F3A\u5EA6`, value: value.reasoningEffort ?? "", disabled: !info && !value.reasoningEffort, onChange: (event) => set2("reasoningEffort", event.target.value) }, /* @__PURE__ */ import_react4.default.createElement("option", { value: "" }, info && !efforts.length ? "\u5F53\u524D\u6A21\u578B\u9ED8\u8BA4" : "\u81EA\u52A8\u7EE7\u627F\u652F\u6301\u7684\u8BBE\u7F6E"), value.reasoningEffort && !efforts.some((effort) => effort.id === value.reasoningEffort) && /* @__PURE__ */ import_react4.default.createElement("option", { value: value.reasoningEffort }, value.reasoningEffort, "\uFF08\u5F53\u524D\u6A21\u578B\u4E0D\u652F\u6301\uFF09"), efforts.map((effort) => /* @__PURE__ */ import_react4.default.createElement("option", { key: effort.id, value: effort.id }, effort.name ?? effort.id))), info && !efforts.length && /* @__PURE__ */ import_react4.default.createElement("span", { className: "tavern-caption" }, "\u6B64\u6A21\u578B\u4E0D\u63A5\u53D7\u989D\u5916\u63A8\u7406\u5F3A\u5EA6\u53C2\u6570\u3002"), error && /* @__PURE__ */ import_react4.default.createElement("span", { className: "tavern-error" }, error)), /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-grid" }, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u63D0\u4F9B\u65B9", /* @__PURE__ */ import_react4.default.createElement("select", { "aria-label": `${name}\u63D0\u4F9B\u65B9`, value: value.provider, onChange: (event) => set2("provider", event.target.value) }, /* @__PURE__ */ import_react4.default.createElement("option", { value: "" }, "\u8DDF\u968F\u8F93\u5165\u6846\u6A21\u578B"), providers.map((provider) => /* @__PURE__ */ import_react4.default.createElement("option", { key: provider.provider, value: provider.provider }, provider.displayName ?? provider.provider)))), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u6A21\u578B", /* @__PURE__ */ import_react4.default.createElement("input", { "aria-label": `${name}\u6A21\u578B`, list: `tavern-model-${name}`, value: value.model, placeholder: "\u7559\u7A7A\u8DDF\u968F\u8F93\u5165\u6846", onChange: (event) => set2("model", event.target.value) }), /* @__PURE__ */ import_react4.default.createElement("datalist", { id: `tavern-model-${name}` }, models.map((model) => /* @__PURE__ */ import_react4.default.createElement("option", { key: model.id, value: model.id }, model.name))))), !compact && /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u53C2\u6570\u4E0E\u9644\u52A0\u63D0\u793A\u8BCD"), /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-grid" }, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u6E29\u5EA6", /* @__PURE__ */ import_react4.default.createElement("input", { type: "number", min: "0", max: "2", step: "0.05", value: value.temperature ?? "", placeholder: "\u7EE7\u627F\u9884\u8BBE", onChange: (event) => set2("temperature", event.target.value === "" ? null : Number(event.target.value)) })), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u7ED3\u6784\u5316\u7ED3\u679C", /* @__PURE__ */ import_react4.default.createElement("select", { value: value.protocol, onChange: (event) => set2("protocol", event.target.value) }, /* @__PURE__ */ import_react4.default.createElement("option", { value: "tool" }, "\u5DE5\u5177\u8C03\u7528"), /* @__PURE__ */ import_react4.default.createElement("option", { value: "json" }, "JSON \u534F\u8BAE\u5757")))), /* @__PURE__ */ import_react4.default.createElement("textarea", { "aria-label": `${name}\u9644\u52A0\u63D0\u793A\u8BCD`, rows: "3", value: value.prompt, onChange: (event) => set2("prompt", event.target.value) })));
}
function PresetEditor({ id, sessionId, scope, override, saved }) {
  const [preset, setPreset] = (0, import_react4.useState)(), [query, setQuery] = (0, import_react4.useState)(""), [error, setError] = (0, import_react4.useState)("");
  (0, import_react4.useEffect)(() => {
    setPreset(void 0);
    if (override) setPreset(clone(override));
    else if (id) api("/item?id=" + id).then((item) => setPreset(item.data)).catch((error2) => setError(error2.message));
  }, [id, sessionId, override]);
  if (!id) return null;
  const update = (id2, work) => {
    const next = clone(preset);
    work(next.prompts.find((p) => (p.identifier ?? p.id) === id2), (next.prompt_order.find((order) => order.character_id === 100001) ?? next.prompt_order[0]).order);
    setPreset(next);
  };
  return /* @__PURE__ */ import_react4.default.createElement("details", { className: "tavern-preset-editor" }, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u7F16\u8F91\u9884\u8BBE\u63D0\u793A\u8BCD"), error && /* @__PURE__ */ import_react4.default.createElement("p", { role: "alert" }, error), preset && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, /* @__PURE__ */ import_react4.default.createElement("input", { type: "search", "aria-label": "\u67E5\u627E\u9884\u8BBE\u63D0\u793A\u8BCD", value: query, onChange: (event) => setQuery(event.target.value), placeholder: "\u67E5\u627E\u63D0\u793A\u8BCD" }), presetPrompts(preset).filter((prompt) => !query || prompt.name?.includes(query)).map((prompt) => /* @__PURE__ */ import_react4.default.createElement("details", { key: prompt.id }, /* @__PURE__ */ import_react4.default.createElement("summary", null, /* @__PURE__ */ import_react4.default.createElement("input", { type: "checkbox", "aria-label": `\u542F\u7528 ${prompt.name}`, checked: prompt.enabled, onClick: (event) => event.stopPropagation(), onChange: (event) => update(prompt.id, (_entry, order) => {
    order.find((item) => item.identifier === prompt.id).enabled = event.target.checked;
  }) }), " ", prompt.name || prompt.id), !prompt.marker && /* @__PURE__ */ import_react4.default.createElement("textarea", { rows: "5", value: prompt.content ?? "", onChange: (event) => update(prompt.id, (entry) => {
    entry.content = event.target.value;
  }) }), /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-actions" }, [-1, 1].map((delta) => /* @__PURE__ */ import_react4.default.createElement("button", { key: delta, onClick: () => update(prompt.id, (_entry, order) => {
    const i = order.findIndex((item) => item.identifier === prompt.id), j2 = i + delta;
    if (j2 >= 0 && j2 < order.length) [order[i], order[j2]] = [order[j2], order[i]];
  }) }, delta === -1 ? "\u4E0A\u79FB" : "\u4E0B\u79FB"))))), /* @__PURE__ */ import_react4.default.createElement("button", { onClick: async () => {
    try {
      await api("/item", { id, data: preset, sessionId, scope });
      await saved();
      setError("");
    } catch (error2) {
      setError(error2.message);
...
function createSettings(runtime) {
  ...
  return function TavernSettings({ useSessions, sessionId: boundSessionId, embedded = false, initialPage = "resources", scope = "session" }) {
    ...
    const current = useSessions((list) => list.byId[list.current]?.projectionValues?.agentPreset === "tavern" ? list.current : void 0);
    const sessionId = global2 ? void 0 : boundSessionId ?? current;
    const available = global2 ? globalPages : pages;
    const [page, setPage] = (0, import_react4.useState)(available[initialPage] ? initialPage : "resources"), [data, setData] = (0, import_react4.useState)(), [payload, setPayload] = (0, import_react4.useState)(), [selection, setSelection] = (0, import_react4.useState)({});
    const [config, setConfig] = (0, import_react4.useState)(), [message, setMessage] = (0, import_react4.useState)(""), [error, setError] = (0, import_react4.useState)(false), [busy, setBusy] = (0, import_react4.useState)(false);
    const [configurationId, setConfigurationId] = (0, import_react4.useState)(""), [configuration, setConfiguration] = (0, import_react4.useState)();
    const [history, setHistory] = (0, import_react4.useState)([]), [legacyPath, setLegacyPath] = (0, import_react4.useState)("");
    const [actorId, setActorId] = (0, import_react4.useState)(""), [actor, setActor] = (0, import_react4.useState)(), [tableId, setTableId] = (0, import_react4.useState)(""), [tables, setTables] = (0, import_react4.useState)({}), [variables, setVariables] = (0, import_react4.useState)("{}");
    async function load() {
      const [data2, configurations] = await Promise.all([runtime.bootstrap(true), api("/configurations")]);
      setData({ ...data2, configurations });
      setConfigurationId("");
      setConfiguration(void 0);
      let payload2 = sessionId ? await api("/native-state?id=" + sessionId) : void 0;
      setPayload(payload2);
      setSelection(payload2?.state ? fromState(payload2.state) : global2 ? { ...data2.selection } : { ...payload2?.selection });
      setConfig(clone(payload2?.state?.config ?? payload2?.config ?? data2.config));
      setHistory(payload2?.state ? await api("/messages?id=" + sessionId) : []);
      setTables(clone(payload2?.state?.tables ?? {}));
      setVariables(JSON.stringify(payload2?.state?.variables ?? {}, null, 2));
      return payload2;
    }
    (0, import_react4.useEffect)(() => {
      load().catch((error2) => {
    ...
    (0, import_react4.useEffect)(() => {
      const list = payload?.state?.characters ?? [];
      const current2 = list.find((actor2) => actor2.id === actorId) ?? list[0];
      setActorId(current2?.id ?? "");
      setActor(current2 ? clone(current2) : void 0);
    }, [payload, actorId]);
    async function action(work, success = "\u5DF2\u4FDD\u5B58") {
      setBusy(true);
      setMessage("");
      try {
        await work();
    ...
    async function refresh() {
      if (sessionId) await runtime.refresh(sessionId, true);
      await load();
    }
    async function saveSelection() {
      ...
      const id = global2 ? (await api("/native-selection", { selection: next, configuration }), void 0) : await runtime.selectCard(sessionId, next, configuration);
    ...
    if (!data || !config) return /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-settings" }, message || "\u6B63\u5728\u8BFB\u53D6\u9152\u9986\u8BBE\u7F6E\u2026");
    ...
    return /* @__PURE__ */ import_react4.default.createElement("section", { className: "tavern-settings" }, !embedded && /* @__PURE__ */ import_react4.default.createElement("h2", null, "\u9152\u9986\u6A21\u5F0F"), global2 && /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-caption" }, "\u65B0\u5BF9\u8BDD\u7684\u9ED8\u8BA4\u914D\u7F6E\uFF1B\u5F53\u524D\u5BF9\u8BDD\u7684\u914D\u7F6E\u5728\u8F93\u5165\u6846\u4E0A\u65B9\u4FEE\u6539\u3002"), /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-submenu", role: "tablist", "aria-label": "\u9152\u9986\u8BBE\u7F6E" }, Object.entries(available).map(([key, label]) => /* @__PURE__ */ import_react4.default.createElement("button", { role: "tab", key, "aria-selected": page === key, onClick: () => {
      setPage(key);
      setMessage("");
    } }, label))), message && /* @__PURE__ */ import_react4.default.createElement("p", { role: error ? "alert" : "status", className: error ? "tavern-error" : "tavern-notice" }, message), /* @__PURE__ */ import_react4.default.createElement("fieldset", { disabled: busy, className: "tavern-page", role: "tabpanel" }, page === "resources" && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u5386\u53F2\u914D\u7F6E", /* @__PURE__ */ import_react4.default.createElement("select", { "aria-label": "\u5386\u53F2\u914D\u7F6E", value: configurationId, onChange: (event) => {
      const profile = data.configurations.find((profile2) => profile2.id === event.target.value);
      setConfigurationId(event.target.value);
      if (profile) {
        setSelection(clone(profile.selection));
        setConfig(clone(profile.configuration.config));
        setConfiguration(clone(profile.configuration));
      } else setConfiguration(void 0);
    } }, /* @__PURE__ */ import_react4.default.createElement("option", { value: "" }, "\u9009\u62E9\u4E4B\u524D\u4F7F\u7528\u8FC7\u7684\u914D\u7F6E"), data.configurations.map((profile) => /* @__PURE__ */ import_react4.default.createElement("option", { key: profile.id, value: profile.id }, profile.name, " \xB7 ", new Date(profile.updatedAt).toLocaleString())))), /* @__PURE__ */ import_react4.default.createElement(ResourceImporter, { imported: load }), /* @__PURE__ */ import_react4.default.createElement(ResourceSelect, { label: "\u89D2\u8272\u5361", kind: "card", required: true, value: selection.cardId, change: change("cardId"), library: data.library }), /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-grid" }, /* @__PURE__ */ import_react4.default.createElement(ResourceSelect, { label: "\u5199\u4F5C\u9884\u8BBE", kind: "preset", value: selection.presetId, change: change("presetId"), library: data.library }, "SillyTavern Default"), /* @__PURE__ */ import_react4.default.createElement(ResourceSelect, { label: "\u5DE5\u5177\u9884\u8BBE", kind: "preset", value: selection.toolPresetId, change: change("toolPresetId"), library: data.library }, "\u4E0D\u4F7F\u7528"), /* @__PURE__ */ import_react4.default.createElement(ResourceSelect, { label: "\u63A8\u8FDB\u4E0E\u586B\u8868\u9884\u8BBE", kind: "advance", value: selection.legacyId, change: change("legacyId"), library: data.library }), /* @__PURE__ */ import_react4.default.createElement(ResourceSelect, { label: "\u8868\u683C\u6A21\u677F", kind: "tables", value: selection.tableId, change: change("tableId"), library: data.library }, "\u968F\u89D2\u8272\u5361")), global2 && /* @__PURE__ */ import_react4.default.createElement("label", null, "\u4F60\u7684\u540D\u5B57", /* @__PURE__ */ import_react4.default.createElement("input", { value: selection.userName ?? "\u4F60", onChange: (event) => change("userName")(event.target.value) })), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u4F60\u7684\u89D2\u8272\u8BBE\u5B9A", /* @__PURE__ */ import_react4.default.createElement("textarea", { rows: "3", value: selection.persona ?? "", onChange: (event) => change("persona")(event.target.value) })), /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u9644\u52A0\u4E16\u754C\u4E66\u4E0E\u6B63\u5219"), [["worldbookIds", "worldbook", "\u4E16\u754C\u4E66"], ["regexIds", "regex", "\u6B63\u5219"]].map(([key, kind, name]) => /* @__PURE__ */ import_react4.default.createElement("label", { key }, name, /* @__PURE__ */ import_react4.default.createElement("select", { multiple: true, value: selection[key] ?? [], onChange: (event) => change(key)([...event.target.selectedOptions].map((option) => option.value)) }, data.library.filter((item) => item.kind === kind).map((item) => /* @__PURE__ */ import_react4.default.createElement("option", { key: item.id, value: item.id }, item.name)))))), /* @__PURE__ */ import_react4.default.createElement("button", { className: "tavern-save", onClick: () => action(saveSelection) }, "\u4FDD\u5B58\u9009\u62E9"), /* @__PURE__ */ import_react4.default.createElement(PresetEditor, { id: selection.presetId, sessionId, scope, override: selection.presetId === (state?.presetId ?? (global2 ? data.selection : payload?.selection)?.presetId) ? state?.presetOverride ?? (global2 ? data : payload)?.presetOverride : void 0, saved: refresh }), /* @__PURE__ */ import_react4.default.createElement(PresetEditor, { id: selection.toolPresetId, saved: refresh }), /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u8BFB\u53D6\u672C\u673A\u9152\u9986\u914D\u7F6E"), /* @__PURE__ */ import_react4.default.createElement("label", null, "SillyTavern \u76EE\u5F55", /* @__PURE__ */ import_react4.default.createElement("input", { value: legacyPath, placeholder: "\u76F8\u5BF9\u9152\u9986\u6570\u636E\u76EE\u5F55\u7684\u8DEF\u5F84", onChange: (event) => setLegacyPath(event.target.value) })), /* @__PURE__ */ import_react4.default.createElement("button", { disabled: !legacyPath.trim(), onClick: () => action(async () => {
      await api("/import-legacy", { path: legacyPath.trim() });
      await load();
    }, "\u63A8\u8FDB\u4E0E\u586B\u8868\u9884\u8BBE\u5DF2\u5BFC\u5165") }, "\u8BFB\u53D6\u672C\u673A\u9152\u9986\u7684\u63A8\u8FDB\u4E0E\u586B\u8868\u9884\u8BBE"))), page === "agents" && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u73A9\u6CD5", /* @__PURE__ */ import_react4.default.createElement("select", { "aria-label": "\u73A9\u6CD5", value: config.playMode ?? "agent", onChange: (event) => setConfig({ ...config, playMode: event.target.value }) }, /* @__PURE__ */ import_react4.default.createElement("option", { value: "agent" }, "\u591A agent"), /* @__PURE__ */ import_react4.default.createElement("option", { value: "normal" }, "\u666E\u901A"))), /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-caption" }, (config.playMode ?? "agent") === "normal" ? "\u666E\u901A\u6A21\u5F0F\u50CF\u9152\u9986\u4E00\u6837\u76F4\u63A5\u7528\u5199\u4F5C\u9884\u8BBE\u548C\u4E16\u754C\u4E66\u73A9\uFF0C\u4E0D\u62C6 agent\u3001\u4E0D\u53EC\u56DE\u3001\u4E0D\u586B\u8868\u3002" : "\u5404\u9636\u6BB5\u4F1A\u81EA\u52A8\u6267\u884C\u3002\u6A21\u578B\u7559\u7A7A\u65F6\u8DDF\u968F dsh \u8F93\u5165\u6846\u4E2D\u7684\u9009\u62E9\u3002\u5361\u7247\u4E16\u754C\u4E66\u6309\u9152\u9986\u626B\u63CF\u6CE8\u5165\u5199\u4F5C\uFF0C\u4E0D\u7ECF agent \u6311\u9009\u3002"), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u666E\u901A\u6A21\u5F0F\u6700\u5927\u8F93\u5165\uFF08\u4F30\u7B97 token\uFF09", /* @__PURE__ */ import_react4.default.createElement("input", { type: "number", min: "1", value: config.normalMaxInputTokens ?? 2e5, onChange: (event) => setConfig({ ...config, normalMaxInputTokens: Number(event.target.value) }) })), /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-grid" }, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u6B63\u6587\u76EE\u6807\u5B57\u6570\u4E0B\u9650", /* @__PURE__ */ import_react4.default.createElement("input", { "aria-label": "\u6B63\u6587\u76EE\u6807\u5B57\u6570\u4E0B\u9650", type: "number", min: "1", value: config.writingMinChars ?? DEFAULT_WRITING_LENGTH.min, onChange: (event) => setConfig({ ...config, writingMinChars: Number(event.target.value) }) })), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u6B63\u6587\u76EE\u6807\u5B57\u6570\u4E0A\u9650", /* @__PURE__ */ import_react4.default.createElement("input", { "aria-label": "\u6B63\u6587\u76EE\u6807\u5B57\u6570\u4E0A\u9650", type: "number", min: "1", value: config.writingMaxChars ?? DEFAULT_WRITING_LENGTH.max, onChange: (event) => setConfig({ ...config, writingMaxChars: Number(event.target.value) }) }))), /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-caption" }, "\u5B57\u6570\u533A\u95F4\u4EC5\u5199\u8FDB\u6B63\u6587\u63D0\u793A\u8BCD\uFF0C\u4E0D\u8BA1\u601D\u8003\u3001HTML \u4E0E\u534F\u8BAE\u6570\u636E\u3002\u8D85\u8FC7\u76EE\u6807\u4E5F\u4E0D\u5F3A\u884C\u622A\u65AD\uFF1B\u6301\u7EED\u8F93\u51FA\u6B63\u6587\u3001\u601D\u8003\u6216\u5DE5\u5177\u53C2\u6570\u65F6\u4F1A\u7EE7\u7EED\u7B49\u5F85\u3002"), Object.entries(labels).filter(([key]) => config.playMode !== "normal" || key === "write").map(([key, label]) => /* @__PURE__ */ import_react4.default.createElement("details", { key, open: key === "advance" || key === "write" }, /* @__PURE__ */ import_react4.default.createElement("summary", null, config.playMode === "normal" ? "\u5199\u4F5C\u6A21\u578B" : `${label} agent`), /* @__PURE__ */ import_react4.default.createElement(ResourceSelect, { label: "\u9884\u8BBE", kind: "preset", value: config.agents[key].presetId ?? "", change: (value) => setConfig((previous) => ({ ...previous, agents: { ...previous.agents, [key]: { ...previous.agents[key], presetId: value } } })), library: data.library }, key === "write" ? "\u5199\u4F5C\u9884\u8BBE" : "\u5DE5\u5177\u9884\u8BBE"), /* @__PURE__ */ import_react4.default.createElement(ModelFields, { name: label, value: config.agents[key], providers: data.providers, sessionId, change: (value) => setConfig((previous) => ({ ...previous, agents: { ...previous.agents, [key]: { ...previous.agents[key], ...value } } })) }))), config.playMode !== "normal" && /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-grid" }, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u5E76\u53D1\u6570", /* @__PURE__ */ import_react4.default.createElement("input", { type: "number", min: "1", max: "16", value: config.concurrency, onChange: (event) => setConfig({ ...config, concurrency: Number(event.target.value) }) })), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u8FD1\u671F\u5BF9\u8BDD\u8F6E\u6570", /* @__PURE__ */ import_react4.default.createElement("input", { type: "number", min: "1", value: config.historyTurns, onChange: (event) => setConfig({ ...config, historyTurns: Number(event.target.value) }) })), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u81EA\u52A8\u7EA0\u9519\u91CD\u8BD5\u6B21\u6570", /* @__PURE__ */ import_react4.default.createElement("input", { type: "number", min: "0", max: "3", value: config.protocolRetries, onChange: (event) => setConfig({ ...config, protocolRetries: Number(event.target.value) }) }), /* @__PURE__ */ import_react4.default.createElement("span", { className: "tavern-caption" }, "\u6BCF\u4E2A\u5DE5\u5177\u6216\u63A8\u8FDB\u4EFB\u52A1\u7684\u7ED3\u679C\u6821\u9A8C\u5931\u8D25\u540E\uFF0C\u6700\u591A\u989D\u5916\u7EA0\u9519\u7684\u6B21\u6570\u30023 \u8868\u793A\u9996\u6B21\u8BF7\u6C42\u540E\u6700\u591A\u518D\u7EA0\u9519 3 \u6B21\uFF0C\u6210\u529F\u5373\u505C\u6B62\u3002\u62D2\u7B54\u3001\u65E0\u53EF\u4FEE\u590D\u7ED3\u679C\u3001\u8F93\u51FA\u622A\u65AD\u3001\u9759\u9ED8\u8D85\u65F6\u6216\u53D6\u6D88\u65F6\u505C\u6B62\uFF1B\u7F51\u7EDC\u91CD\u8BD5\u9075\u5FAA\u63D0\u4F9B\u65B9\u8BBE\u7F6E\u3002"))), /* @__PURE__ */ import_react4.default.createElement("button", { className: "tavern-save", onClick: () => action(async () => {
      for (const agent of Object.values(config.agents)) if (!!agent.provider !== !!agent.model) throw new Error("\u63D0\u4F9B\u65B9\u548C\u6A21\u578B\u9700\u4E00\u8D77\u586B\u5199\uFF0C\u6216\u4E00\u8D77\u7559\u7A7A");
      await api("/config", { config, ...!global2 ? { sessionId, revision: state?.revision } : {} });
      await refresh();
    }) }, "\u4FDD\u5B58\u6A21\u578B\u914D\u7F6E"), sessionId && /* @__PURE__ */ import_react4.default.createElement(ModelAttempts, { key: sessionId, sessionId })), page === "messages" && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, history.map((entry, index) => /* @__PURE__ */ import_react4.default.createElement("details", { key: entry.id ?? entry.nativeMessageId }, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u7B2C ", index + 1, " \u697C \xB7 ", entry.role === "user" ? "\u4F60" : entry.role === "assistant" ? "\u6A21\u578B" : "\u65C1\u767D", " \xB7 ", entry.content.slice(0, 40)), /* @__PURE__ */ import_react4.default.createElement(MessageActions, { sessionId, payload, message: entry.id ? entry : void 0, nativeMessageId: entry.nativeMessageId, role: entry.role, text: entry.content, changed: refresh }, /* @__PURE__ */ import_react4.default.createElement("pre", null, entry.content))))), page === "memory" && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, state && actor && /* @__PURE__ */ import_react4.default.createElement(StateFacts, { episodes: state.memories[actor.id], title: "\u5F53\u524D\u72B6\u6001\u4E0E\u5DF2\u77E5\u4FE1\u606F" }), !state ? /* @__PURE__ */ import_react4.default.createElement("p", null, "\u5F00\u59CB\u9152\u9986\u4F1A\u8BDD\u540E\u53EF\u67E5\u770B\u89D2\u8272\u8BB0\u5FC6\u3002") : /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u89D2\u8272", /* @__PURE__ */ import_react4.default.createElement("select", { value: actorId, onChange: (event) => setActorId(event.target.value) }, state.characters.map((actor2) => /* @__PURE__ */ import_react4.default.createElement("option", { key: actor2.id, value: actor2.id }, actor2.name)))), !actor && /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-caption" }, "\u5267\u60C5\u63A8\u8FDB\u65F6\u4F1A\u8BC6\u522B\u51FA\u573A\u89D2\u8272\uFF1B\u540E\u7EED\u51FA\u73B0\u7684\u65B0\u89D2\u8272\u4E5F\u4F1A\u5EFA\u7ACB\u72EC\u7ACB\u8BB0\u5FC6\u3002"), actor && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u89D2\u8272\u8BBE\u7F6E"), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u59D3\u540D", /* @__PURE__ */ import_react4.default.createElement("input", { value: actor.name, onChange: (event) => setActor({ ...actor, name: event.target.value }) })), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u57FA\u7840\u8BBE\u5B9A", /* @__PURE__ */ import_react4.default.createElement("textarea", { rows: "5", value: actor.profile ?? "", onChange: (event) => setActor({ ...actor, profile: event.target.value }) })), /* @__PURE__ */ import_react4.default.createElement("label", { className: "tavern-check" }, /* @__PURE__ */ import_react4.default.createElement("input", { type: "checkbox", checked: actor.enabled !== false, onChange: (event) => setActor({ ...actor, enabled: event.target.checked }) }), "\u53C2\u4E0E\u5267\u60C5"), [["recallModel", "\u89D2\u8272\u53EC\u56DE"], ["memoryModel", "\u89D2\u8272\u8BB0\u5FC6"]].map(([key, name]) => /* @__PURE__ */ import_react4.default.createElement(ModelFields, { key, compact: true, name, value: actor[key] ?? { provider: "", model: "" }, providers: data.providers, sessionId, change: (value) => setActor({ ...actor, [key]: value }) })), /* @__PURE__ */ import_react4.default.createElement("button", { onClick: () => action(async () => {
      const next = { ...actor };
      for (const key of ["recallModel", "memoryModel"]) {
        if (!next[key]?.provider && !next[key]?.model) delete next[key];
        else if (!next[key]?.provider || !next[key]?.model) throw new Error("\u89D2\u8272\u6A21\u578B\u9700\u540C\u65F6\u586B\u5199\u63D0\u4F9B\u65B9\u4E0E\u6A21\u578B");
      }
      await api("/session", { id: sessionId, revision: state.revision, fields: { characters: state.characters.map((item) => item.id === next.id ? next : item) } });
      await refresh();
    }) }, "\u4FDD\u5B58\u89D2\u8272\u8BBE\u7F6E")), memoryEpisodes(state, actor.id).toReversed().map((memory) => /* @__PURE__ */ import_react4.default.createElement("details", { key: memory.id }, /* @__PURE__ */ import_react4.default.createElement("summary", null, memory.summary || new Date(memory.createdAt).toLocaleString()), memory.eventIds?.length > 0 && /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-caption" }, "\u5F15\u7528 ", memory.eventIds.length, " \u6761\u4E8B\u4EF6\uFF1B\u4E8B\u4EF6\u539F\u6587\u53EA\u5B58\u4E00\u4EFD\uFF0C\u672C\u89D2\u8272\u6309\u77E5\u60C5\u8303\u56F4\u8BFB\u53D6\u3002"), (memory.facts ?? []).map((fact, index) => /* @__PURE__ */ import_react4.default.createElement("p", { key: index }, fact)), (memory.relationships ?? []).map((fact, index) => /* @__PURE__ */ import_react4.default.createElement("p", { key: "r" + index }, fact)), (memory.openThreads ?? []).map((fact, index) => /* @__PURE__ */ import_react4.default.createElement("p", { key: "t" + index }, fact)))), !state.memories[actor.id]?.length && /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-caption" }, "\u5267\u60C5\u5B8C\u6210\u540E\u4F1A\u81EA\u52A8\u8BB0\u5F55\u6B64\u89D2\u8272\u7684\u7ECF\u5386\u3002")))), page === "tables" && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, state && /* @__PURE__ */ import_react4.default.createElement(StateFacts, { episodes: state.worldHistory, title: "\u5F53\u524D\u4E16\u754C\u72B6\u6001" }), !selectedTable ? /* @__PURE__ */ import_react4.default.createElement("p", null, "\u9009\u62E9\u89D2\u8272\u5361\u540E\u5373\u53EF\u4F7F\u7528\u5185\u7F6E\u8868\u683C\u3002") : /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, /* @__PURE__ */ import_react4.default.createElement("label", null, "\u8868\u683C", /* @__PURE__ */ import_react4.default.createElement("select", { value: selectedTable[0], onChange: (event) => setTableId(event.target.value) }, tableEntries.map(([key, table]) => /* @__PURE__ */ import_react4.default.createElement("option", { key, value: key }, table.name)))), /* @__PURE__ */ import_react4.default.createElement("div", { className: "tavern-table" }, /* @__PURE__ */ import_react4.default.createElement("table", null, /* @__PURE__ */ import_react4.default.createElement("thead", null, /* @__PURE__ */ import_react4.default.createElement("tr", null, selectedTable[1].content[0].map((header, index) => /* @__PURE__ */ import_react4.default.createElement("th", { key: index }, header)))), /* @__PURE__ */ import_react4.default.createElement("tbody", null, selectedTable[1].content.slice(1).map((row, rowIndex) => /* @__PURE__ */ import_react4.default.createElement("tr", { key: rowIndex }, row.map((value, column) => /* @__PURE__ */ import_react4.default.createElement("td", { key: column }, /* @__PURE__ */ import_react4.default.createElement("input", { "aria-label": `${selectedTable[1].content[0][column]} \u7B2C${rowIndex + 1}\u884C`, value: value ?? "", onChange: (event) => {
      const next = clone(tables);
      next[selectedTable[0]].content[rowIndex + 1][column] = typeof value === "number" ? Number(event.target.value) : event.target.value;
      setTables(next);
    } })))))))), /* @__PURE__ */ import_react4.default.createElement("button", { className: "tavern-save", onClick: () => action(async () => {
      await api("/session", { id: sessionId, revision: state.revision, fields: { tables } });
      await refresh();
    }) }, "\u4FDD\u5B58\u8868\u683C\u4FEE\u6539"), /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u6A21\u677F\u89C4\u5219"), /* @__PURE__ */ import_react4.default.createElement("pre", null, JSON.stringify(selectedTable[1].sourceData, null, 2))))), page === "frontend" && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, data.helper && /* @__PURE__ */ import_react4.default.createElement("div", null, /* @__PURE__ */ import_react4.default.createElement("p", null, "\u9152\u9986\u52A9\u624B ", data.helper.version, " \xB7 ", /* @__PURE__ */ import_react4.default.createElement("a", { href: data.helper.repository, target: "_blank", rel: "noreferrer" }, "\u5B98\u65B9\u6E90\u7801")), global2 && /* @__PURE__ */ import_react4.default.createElement("label", { className: "tavern-check" }, /* @__PURE__ */ import_react4.default.createElement("input", { type: "checkbox", checked: data.helper.autoUpdate, onChange: (event) => action(async () => {
      await api("/helper", { autoUpdate: event.target.checked });
      await load();
    }) }), "\u81EA\u52A8\u68C0\u67E5\u5E76\u66F4\u65B0"), /* @__PURE__ */ import_react4.default.createElement("button", { onClick: () => action(async () => {
      await api("/helper", { check: true });
      await load();
    }, "\u5DF2\u68C0\u67E5\u9152\u9986\u52A9\u624B\u66F4\u65B0") }, "\u68C0\u67E5\u66F4\u65B0"), data.helper.checkedAt && /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-caption" }, "\u4E0A\u6B21\u68C0\u67E5\uFF1A", new Date(data.helper.checkedAt).toLocaleString()), data.helper.error && /* @__PURE__ */ import_react4.default.createElement("p", { className: "tavern-error", role: "alert" }, data.helper.error)), /* @__PURE__ */ import_react4.default.createElement("label", null, "\u663E\u793A\u65B9\u5F0F", /* @__PURE__ */ import_react4.default.createElement("select", { value: selection.renderMode ?? "card", onChange: (event) => change("renderMode")(event.target.value) }, /* @__PURE__ */ import_react4.default.createElement("option", { value: "card" }, "\u89D2\u8272\u5361\u6B63\u5219\u4E0E\u524D\u7AEF\u7EC4\u4EF6"), /* @__PURE__ */ import_react4.default.createElement("option", { value: "bubble" }, "\u5185\u7F6E\u6C14\u6CE1"), /* @__PURE__ */ import_react4.default.createElement("option", { value: "text" }, "\u539F\u59CB\u6587\u672C"))), /* @__PURE__ */ import_react4.default.createElement("button", { className: "tavern-save", onClick: () => action(saveSelection) }, "\u4FDD\u5B58\u663E\u793A\u65B9\u5F0F"), state && /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u804A\u5929\u53D8\u91CF"), /* @__PURE__ */ import_react4.default.createElement("textarea", { className: "tavern-code", rows: "10", "aria-label": "\u804A\u5929\u53D8\u91CF JSON", value: variables, onChange: (event) => setVariables(event.target.value) }), /* @__PURE__ */ import_react4.default.createElement("button", { onClick: () => action(async () => {
      await api("/runtime", { sessionId, method: "variables", args: { variables: JSON.parse(variables) } });
      await refresh();
    }) }, "\u4FDD\u5B58\u53D8\u91CF")), /* @__PURE__ */ import_react4.default.createElement("details", null, /* @__PURE__ */ import_react4.default.createElement("summary", null, "\u89D2\u8272\u811A\u672C"), /* @__PURE__ */ import_react4.default.createElement("button", { onClick: () => action(() => runtime.script(sessionId), "") }, "\u6253\u5F00\u811A\u672C\u754C\u9762"), (runtime.controller(sessionId)?.buttons ?? []).map((button) => /* @__PURE__ */ import_react4.default.createElement("button", { key: button.id, onClick: () => action(() => runtime.script(sessionId, button.id), "") }, button.name)))))));
  };
}
    ...
    while (offset < inputLength) {
      while ((current & (BinTrieFlags.VALUE_LENGTH | BinTrieFlags.FLAG13)) === 0 && (current & BinTrieFlags.JUMP_TABLE) !== 0) {
        ...
        if (branchCount === 0) {
          ...
          const stored2 = decodeTree[treeIndex + 1 + slot];
          if (stored2 === 0) {
            return this.flushAndEmitLegacyOrReject(consumed, excess, char2, 0);
          }
          treeIndex = treeIndex + branchCount + stored2 & 65535;
        }
...
function determineBranch(decodeTree, current, nodeIndex, char) {
  ...
  if (jumpOffset) {
    ...
    if (slot >>> 0 >= branchCount)
      return -1;
    const stored2 = decodeTree[nodeIndex + slot];
    return stored2 === 0 ? -1 : nodeIndex + branchCount + stored2 - 1 & 65535;
  }
  if (branchCount === 0)
    return -1;
...
var b = class u2 {
  ...
  textRenderer;
  constructor(e) {
    this.options = e || O, this.options.renderer = this.options.renderer || new y(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new $2();
  }
  static parse(e, t) {
...
// src/render-segments.js
var blocks = new Set("address article aside blockquote canvas details dialog div dl fieldset figure footer form h1 h2 h3 h4 h5 h6 header hr iframe main nav ol p pre script section style table ul svg video audio template".split(" "));
var containsBlock = (node) => blocks.has(node.tagName) || (node.childNodes ?? []).some(containsBlock);
...
function unfenced(source) {
  ...
  const literals = [];
  g.walkTokens(g.lexer(source), (token) => {
  ...
  for (const node of parseFragment(source, { sourceCodeLocationInfo: true }).childNodes) {
    const location2 = node.sourceCodeLocation;
    if (!location2 || literals.some(([start, end]) => location2.startOffset >= start && location2.startOffset < end)) continue;
    if (node.nodeName === "#comment") {
      ranges.push({ start: location2.startOffset, end: location2.endOffset, block: true, comment: true });
      continue;
    }
    if (!node.tagName) continue;
    const block = containsBlock(node);
    const previous = ranges.at(-1);
    if (previous && !previous.comment && previous.block === block && !source.slice(previous.end, location2.startOffset).trim()) previous.end = location2.endOffset;
    else ranges.push({ start: location2.startOffset, end: location2.endOffset, block });
  }
  const result = [];
  let offset = 0;
  for (const range of ranges.filter((range2) => range2.block)) {
    if (range.start > offset) result.push({ text: source.slice(offset, range.start) });
    if (!range.comment) result.push({ html: source.slice(range.start, range.end) });
    offset = range.end;
  }
...
function renderSegments(text) {
  ...
  while (match = opening.exec(text)) {
    ...
    if (!end) {
      result.push(...unfenced(text.slice(offset, match.index)), { text: text.slice(match.index) });
      offset = text.length;
      break;
    }
  ...
  return result.reduce((segments, part) => {
    if (part.text !== void 0 && segments.at(-1)?.text !== void 0) segments.at(-1).text += part.text;
...
// src/speech-frame.js
var SPEECH_LINE = /\[角色\|([^\]|]+)\|([^\]|]+)\](?:〖([^〗]*)〗|\{([^}]*)\})/g;
var OLD_LINE = /^@bubble:([^|\n]+)\|([^|\n]+)\|\[([^\]]*)\]\s*$/;
function superellipseClip(n = 3, samples = 48) {
  const pts = [];
  for (let i = 0; i <= samples; i++) {
    const t = i / samples * Math.PI * 2;
    const c = Math.cos(t), s = Math.sin(t);
    const x2 = Math.sign(c) * Math.abs(c) ** (2 / n);
    const y2 = Math.sign(s) * Math.abs(s) ** (2 / n);
    pts.push(`${((x2 + 1) * 50).toFixed(2)}% ${((y2 + 1) * 50).toFixed(2)}%`);
  }
...
function hasSpeechMarks(text) {
  return /\[角色\|/.test(text) || /^@bubble:/m.test(text) || /<now_plot>/i.test(text);
}
function withoutLegacyBubble(scripts) {
...
function isPlayerName(name, { userName = "", aliases = "" } = {}) {
  ...
  for (const item of String(aliases).split(/[,，\n]/).map((part) => part.trim()).filter(Boolean)) {
    if (item.startsWith("/") && item.lastIndexOf("/") > 1) {
      ...
      try {
        if (new RegExp(item.slice(1, last), item.slice(last + 1)).test(value)) return true;
      } catch {
      }
...
function parseSpeech(text) {
  ...
  const pushNarration = (chunk) => {
    ...
    if (value.trim()) blocks2.push({ kind: "narration", text: value });
  };
  const takeLine = (start, length, name, mood, speech, thought) => {
    pushNarration(source.slice(cursor, start));
    blocks2.push({ kind: "say", name: name.trim(), mood: mood.trim(), ...parseThought(speech, thought) });
    cursor = start + length;
  };
  ...
  if (!blocks2.some((block) => block.kind === "say")) {
...
function buildSpeechDocument(text, { cardId = "", userName = "", aliases = "", keepCardName = false } = {}) {
...
// src/display.js
function displaySegments(text, enabled, meta = {}) {
  ...
  return renderSegments(text).flatMap((part) => {
    if (part.html !== void 0) return [part];
    if (!hasSpeechMarks(part.text)) return [part];
    return [{ html: buildSpeechDocument(part.text, meta) }];
  });
}

// src/native-recovery-ui.jsx
var import_react5 = __toESM(require("react"), 1);
function createRecoveryNotice(runtime) {
  return function RecoveryNotice({ sessionId, payload, ready }) {
    const submitting = (0, import_react5.useRef)(false);
    const [busy, setBusy] = (0, import_react5.useState)(false), [error, setError] = (0, import_react5.useState)("");
    ...
    async function resume() {
      ...
      setError("");
      try {
        await runtime.retryFailed(sessionId, recovery.token);
      } catch (error2) {
        setError(error2.message);
      } finally {
    ...
    return /* @__PURE__ */ import_react5.default.createElement("div", { className: "tavern-recovery tavern-settings", role: "status" }, /* @__PURE__ */ import_react5.default.createElement("span", null, recovery.kind === "updates" ? "\u6B63\u6587\u5DF2\u4FDD\u5B58\uFF0C\u66F4\u65B0\u672A\u5B8C\u6210\u3002" : "\u672C\u8F6E\u5C1A\u672A\u5B8C\u6210\u3002", recovery.completedSteps > 0 ? `\u5DF2\u4FDD\u5B58 ${recovery.completedSteps} \u4E2A\u6B65\u9AA4\uFF0C\u7EE7\u7EED\u65F6\u4F1A\u590D\u7528\u3002` : "\u5C06\u4ECE\u672A\u5B8C\u6210\u7684\u6B65\u9AA4\u7EE7\u7EED\u3002"), /* @__PURE__ */ import_react5.default.createElement("button", { disabled: busy || !ready, onClick: resume }, busy ? "\u6B63\u5728\u7EE7\u7EED\u2026" : label), error && /* @__PURE__ */ import_react5.default.createElement("p", { className: "tavern-error", role: "alert" }, error));
  };
}

// src/native-opening.jsx
var import_react6 = __toESM(require("react"), 1);
function createOpening(renderMessage) {
  return function Opening({ sessionId, payload, greetingOnly }) {
    const messages = payload.state.messages;
    if (greetingOnly) return messages[0]?.greeting ? /* @__PURE__ */ import_react6.default.createElement("div", { className: "tavern-opening tavern-greeting" }, renderMessage(messages[0], payload, 0, sessionId)) : null;
    let start = messages.length;
    while (start > 0 && messages[start - 1].scriptCreated) start--;
    if (start === messages.length) return null;
    return /* @__PURE__ */ import_react6.default.createElement("div", { className: "tavern-opening" }, messages.slice(start).map((message, offset) => renderMessage(message, payload, start + offset, sessionId)));
  };
}
...
var INTERACTION = {
  ...
  required: ["status", "choices"],
  properties: Object.fromEntries([["status", "value"], ["choices", "message"]].map(([key, field]) => [key, {
    type: "array",
    items: { type: "object", additionalProperties: false, required: ["label", field], properties: { label: { type: "string" }, [field]: { type: "string" } } }
  }]))
};
function extractInteraction(text) {
  ...
  let panel;
  try {
    panel = JSON.parse(matches[0][1]);
  } catch {
    throw new Error("\u4EA4\u4E92\u534F\u8BAE\u4E0D\u662F\u6709\u6548 JSON");
  }
...
function interactionHtml(panel) {
...
// src/native-client.jsx
var inject = ["slots", "sessions", "conversation", "workspaces", "uiWorkspace"];
var markdownLabels = { copy: "\u590D\u5236", copied: "\u5DF2\u590D\u5236", code: "\u4EE3\u7801", open: "\u6253\u5F00", download: "\u4E0B\u8F7D" };
function apply(ctx) {
  ...
  style.dataset.plugin = "dsh-tavern-mode";
  ...
  const usePayload = (id) => (0, import_react7.useSyncExternalStore)((callback) => runtime.subscribe(id, callback), () => runtime.get(id));
  function Component({ sessionId, source, index }) {
    ...
    return /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, null, error && /* @__PURE__ */ import_react7.default.createElement("p", { role: "alert" }, error), ready && /* @__PURE__ */ import_react7.default.createElement("iframe", { key: runtimeId, ref, className: "tavern-native-component", title: "\u89D2\u8272\u5361\u524D\u7AEF\u7EC4\u4EF6", allow: "clipboard-write", sandbox: "allow-scripts allow-same-origin allow-modals allow-downloads" }));
  }
  function renderText(text, payload, index, sessionId, native, placement = 2) {
    ...
    text = display.text;
    if (mode === "text") return /* @__PURE__ */ import_react7.default.createElement("pre", { style: { whiteSpace: "pre-wrap", font: "inherit" } }, text);
    const interaction = placement === 2 ? extractInteraction(text) : { text, panel: null };
    text = interaction.text;
    const env = macroEnvironment({ user: state.userName, char: payload.card.name, variables: state.variables, globalVariables: state.globalVariables, characterVariables: state.characterVariables, presetVariables: state.presetVariables, messageVariables: state.messageVariables, messageId: index < 0 ? void 0 : index, messageSwipes: Object.fromEntries((state.helperChat ?? []).map((message, index2) => [index2, message.swipe_id ?? 0])), messages: state.messages, sessionId });
    ...
    return parts.map((part, key) => part.html !== void 0 ? /* @__PURE__ */ import_react7.default.createElement(Component, { key, sessionId, source: part.html, index }) : /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, { key }, native(part.text)));
  }
  for (const key of ["user", "steering"]) ctx.slots.inject("conversation.chat.node", () => {
    ...
    return bindComponent(original, (Original) => {
      function User(props) {
        ...
        if (preset !== "tavern" || !payload) return /* @__PURE__ */ import_react7.default.createElement(Original, { ...props });
        ...
        if (text === "/tavern-retry-updates") return /* @__PURE__ */ import_react7.default.createElement("p", { className: "tavern-caption" }, "\u91CD\u8BD5\u672A\u5B8C\u6210\u7684\u8868\u683C\u4E0E\u8BB0\u5FC6\u66F4\u65B0");
        if (text.startsWith("/tavern-resume ")) return /* @__PURE__ */ import_react7.default.createElement("p", { className: "tavern-caption" }, "\u4ECE\u5931\u8D25\u6B65\u9AA4\u7EE7\u7EED");
        ...
        const render = (content) => /* @__PURE__ */ import_react7.default.createElement(Original, { ...props, node: { ...props.node, data: { ...props.node.data, content } } });
        let start = index;
        while (start > 0 && payload.state.messages[start - 1].scriptCreated) start--;
        return /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, null, payload.state.messages.slice(Math.max(0, start), Math.max(0, index)).map((message, offset) => standaloneMessage(message, payload, start + offset, props.sessionId)), /* @__PURE__ */ import_react7.default.createElement(MessageActions, { sessionId: props.sessionId, payload, message: payload.state.messages[index], nativeMessageId: props.node.id, role: "user", text }, renderText(text, payload, index, props.sessionId, (text2) => render([{ type: "text", text: text2 }]), 1), attachments.length > 0 && render(attachments)));
      }
      return User;
    });
  });
  ctx.slots.inject("conversation.chat.node", () => {
    ...
    return bindComponent(original, (Original) => {
      function Assistant(props) {
        ...
        if (props.node.data.status !== "settled") return /* @__PURE__ */ import_react7.default.createElement(Original, { ...props });
        ...
        if (!text) return /* @__PURE__ */ import_react7.default.createElement(Original, { ...props });
        const messageIndex = findMessageIndex(payload.state, props.node.data.finalNode?.messageId, text, "assistant");
        return /* @__PURE__ */ import_react7.default.createElement(MessageActions, { sessionId: props.sessionId, payload, message: payload.state.messages[messageIndex], nativeMessageId: props.node.data.finalNode?.messageId, role: "assistant", text, reasoning: props.node.data.blocks.filter((block) => block.kind === "reasoning").map((block) => block.text ?? "").join("\n") }, renderText(text, payload, messageIndex, props.sessionId, (content) => /* @__PURE__ */ import_react7.default.createElement(Original, { ...props, node: { ...props.node, data: { ...props.node.data, blocks: [{ kind: "text", text: content }] } } })));
      }
      return Assistant;
    });
  });
  function hiddenNode(state, node) {
    return state.hiddenNativeRanges?.some(([start, end]) => node.anchorSeq >= start && node.anchorSeq < end);
  }
  ctx.slots.inject("conversation.chat.node", () => {
    const disposers = ctx.slots.entriesOfSlot("conversation.chat.node").filter((entry) => !["user", "steering", "assistant-step"].includes(entry.options.key)).map((original) => {
      return bindComponent(original, (Original) => {
        function ChatNode(props) {
          ...
          return /* @__PURE__ */ import_react7.default.createElement(Original, { ...props, node: text === void 0 ? props.node : { ...props.node, data: { ...props.node.data, closing: { ...closing, blocks: [{ kind: "text", text }] } } } });
        }
        return ChatNode;
      });
    });
  ...
  function standaloneMessage(message, payload, index, sessionId) {
    return /* @__PURE__ */ import_react7.default.createElement(MessageActions, { key: message.id, sessionId, payload, message, role: message.role, text: message.content }, renderText(message.content, payload, index, sessionId, (text) => /* @__PURE__ */ import_react7.default.createElement(import_dsh_client_ui_primitives.MarkdownText, { text, labels: markdownLabels }), message.role === "user" ? 1 : 2));
  }
  ctx.slots.inject("conversation.input.dock", () => {
    ...
    return ctx.slots.register({ name: "conversation.input.dock", id: "tavern-toolbar", order: 20, locale: original.locale, inject: original.inject }, function Toolbar(props) {
      ...
      return /* @__PURE__ */ import_react7.default.createElement("div", { className: "tavern-toolbar" }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "tavern-toolbar-row" }, /* @__PURE__ */ import_react7.default.createElement(Mode, { ...props, select: (mode) => runtime.changeMode(mode).catch((error) => error.message) }), /* @__PURE__ */ import_react7.default.createElement("select", { "aria-label": "\u73A9\u6CD5", value: payload?.state?.config?.playMode ?? draftConfig?.playMode ?? "agent", onChange: (event) => {
      ...
      } }, /* @__PURE__ */ import_react7.default.createElement("option", { value: "agent" }, "\u591A agent"), /* @__PURE__ */ import_react7.default.createElement("option", { value: "normal" }, "\u666E\u901A")), /* @__PURE__ */ import_react7.default.createElement("button", { className: "tavern-current-card", title: payload?.card.name ?? "\u9009\u62E9\u89D2\u8272\u5361", onClick: () => setModal("cards") }, payload?.card.name ?? "\u89D2\u8272\u5361"), /* @__PURE__ */ import_react7.default.createElement("button", { onClick: () => setModal("resources") }, "\u5BFC\u5165"), /* @__PURE__ */ import_react7.default.createElement("button", { onClick: () => setModal("resources") }, "\u9884\u8BBE"), /* @__PURE__ */ import_react7.default.createElement("button", { onClick: () => setModal("agents") }, (payload?.state?.config?.playMode ?? draftConfig?.playMode) === "normal" ? "\u6700\u5927\u8F93\u5165\u4E0E\u6A21\u578B" : "\u591A agent"), /* @__PURE__ */ import_react7.default.createElement("button", { onClick: () => runtime.script(props.sessionId, "\u53F0\u8BCD\u6846").catch((error) => runtime.report(props.sessionId, error.message)) }, "\u53F0\u8BCD\u6846"), buttons.filter((button) => button.name !== "\u53F0\u8BCD\u6846" && button.name !== "\u5BF9\u8BDD\u6C14\u6CE1").map((button) => /* @__PURE__ */ import_react7.default.createElement("button", { key: button.id, onClick: () => runtime.script(props.sessionId, button.id).catch((error) => runtime.report(props.sessionId, error.message)) }, button.name))), notice && /* @__PURE__ */ import_react7.default.createElement(import_dsh_client_ui_primitives.Toast, { key: notice.id, text: notice.message, holdMs: notice.timeOut === 0 ? 2147483647 : notice.timeOut ?? 3e3, anchor: document.querySelector("[data-composer-card]"), icon: /* @__PURE__ */ import_react7.default.createElement("button", { "aria-label": "\u5173\u95ED\u901A\u77E5", onClick: () => runtime.clearNotice(props.sessionId, notice.id) }, "\xD7"), onDone: () => runtime.clearNotice(props.sessionId, notice.id) }), /* @__PURE__ */ import_react7.default.createElement(import_dsh_client_ui_primitives.Modal, { open: !!modal, onClose: () => setModal(""), title: modal === "cards" ? "\u89D2\u8272\u5361" : "\u672C\u5BF9\u8BDD\u8BBE\u7F6E", closeLabel: "\u5173\u95ED", className: "tavern-settings-dialog" }, modal === "cards" ? /* @__PURE__ */ import_react7.default.createElement(ResourceLibrary, { runtime, sessionId: props.sessionId, selected: () => setModal("") }) : modal && /* @__PURE__ */ import_react7.default.createElement(Settings, { ...props, embedded: true, initialPage: modal, key: modal, scope: "session", sessionId: props.sessionId })));
    });
  });
  ctx.slots.inject("main.conversation", () => {
    ...
    return bindComponent(original, (Original) => {
      function TavernConversation(props) {
        ...
        const shell = (0, import_react7.useRef)(), scroll = (0, import_react7.useRef)(), composer = (0, import_react7.useRef)();
        (0, import_react7.useEffect)(() => {
          ...
          return () => {
            observer.disconnect();
            runtime.mountView(props.sessionId, null);
          };
        }, [props.sessionId]);
        const reason = !payload ? "\u8BF7\u5148\u5BFC\u5165\u5E76\u9009\u62E9\u89D2\u8272\u5361" : !ready ? "\u6B63\u5728\u52A0\u8F7D\u89D2\u8272\u524D\u7AEF\u2026" : void 0;
        const bar = /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, null, session && input && props.renderSlot("conversation.input.dock", { session, input }), props.renderSlot("conversation.composer.bar", { variant: "composer", placeholder: reason ?? "\u53D1\u9001\u6D88\u606F\uFF0C\u7EE7\u7EED\u5267\u60C5\u2026", ...reason ? { blocked: { reason } } : {} }));
        return /* @__PURE__ */ import_react7.default.createElement("div", { className: "tavern-shell", ref: shell }, props.renderSlot("conversation.session.header", {}), /* @__PURE__ */ import_react7.default.createElement("div", { className: "tavern-scroll", "data-conversation-scroll": "", ref: scroll }, !payload ? /* @__PURE__ */ import_react7.default.createElement("div", { className: "tavern-entry" }, error && /* @__PURE__ */ import_react7.default.createElement("p", { role: "alert", className: "tavern-error" }, error), loaded || error ? /* @__PURE__ */ import_react7.default.createElement(ResourceLibrary, { runtime, sessionId: props.sessionId }) : /* @__PURE__ */ import_react7.default.createElement("p", null, "\u6B63\u5728\u8BFB\u53D6\u9152\u9986\u2026")) : /* @__PURE__ */ import_react7.default.createElement(import_react7.default.Fragment, null, /* @__PURE__ */ import_react7.default.createElement(Opening, { sessionId: props.sessionId, payload, greetingOnly: true }), !session?.blank && props.renderSlot("conversation.session", {}), /* @__PURE__ */ import_react7.default.createElement(Opening, { sessionId: props.sessionId, payload }), error && /* @__PURE__ */ import_react7.default.createElement("div", { className: "tavern-opening" }, /* @__PURE__ */ import_react7.default.createElement("p", { className: "tavern-error", role: "alert" }, error), /* @__PURE__ */ import_react7.default.createElement("button", { onClick: () => runtime.refresh(props.sessionId, true) }, "\u91CD\u65B0\u52A0\u8F7D\u524D\u7AEF")))), /* @__PURE__ */ import_react7.default.createElement("div", { className: "tavern-composer", "data-composer-seat": "", ref: composer }, /* @__PURE__ */ import_react7.default.createElement(RecoveryNotice, { sessionId: props.sessionId, payload, ready }), props.renderSlotChain("conversation.composer", { sessionId: props.sessionId, session, pendingInteraction }, { fallback: bar, overlay: true })));
      }
      function Conversation(props) {
        const preset = props.useSessions((list) => list.byId[props.sessionId]?.projectionValues?.agentPreset);
        return preset === "tavern" ? /* @__PURE__ */ import_react7.default.createElement(TavernConversation, { ...props }) : /* @__PURE__ */ import_react7.default.createElement(Original, { ...props });
      }
      return Conversation;
    });
  });
  ...
  const newSession = function(workspaceId) {
    ...
    void runtime.newSession(void 0, workspaceId).catch((error) => runtime.report(current.id, error.message));
  };
  ctx.uiWorkspace.startSession = newSession;
  ctx.effect(() => () => {
    if (ctx.uiWorkspace.startSession === newSession) ctx.uiWorkspace.startSession = originalNewSession;
  });
  ...
  function updateMode() {
    ...
    if (next !== active) {
      active = next;
      if (active) settingsDispose = ctx.slots.inject("settings.section", () => ctx.slots.register({ name: "settings.section", id: "tavern-mode", order: 45, label: () => "\u9152\u9986\u6A21\u5F0F" }, (props) => /* @__PURE__ */ import_react7.default.createElement(Settings, { ...props, scope: "global" })));
      else {
        settingsDispose?.();
    ...
    for (const id of keep) {
      runtime.ensure(id);
      runtime.setRunning(id, !!list.byId[id].running);
    }
  }
  function observeSeat() {
    ...
    if (store !== seatStore) {
      stopSeat?.();
      seatStore = store;
      stopSeat = seatStore?.subscribe(updateMode);
    }
    updateMode();
  }
